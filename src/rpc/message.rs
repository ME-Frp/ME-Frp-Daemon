...
impl RpcResponse {
  ...
  pub fn new(id: String, result: Option<Value>, error: Option<Value>) -> Self {
  ...
  pub fn from_value(id: String, value: Value) -> Self {
    debug!("Success response: id={}", id);
    trace!("Value: {:?}", value);
    Self { id: Some(id), result: Some(value), error: None }
  }
  ...
  pub fn from_result(id: String, result: Result<Value, anyhow::Error>) -> Self {
    match result {
      Ok(value) => Self::from_value(id, value),
